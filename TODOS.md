...
- [x] **Live search / filter** — press `/` to open an `Input` widget that live-filters the DataTable (Series title, Story title, Issue #, LGY #). Esc clears it. No suspend, no API calls.
- [x] **Delete confirmation modal** — `DeleteConfirmScreen(ModalScreen)`: shows issue label, `[D]elete` / `[Esc]cancel`. Calls `db.delete_issue()` directly on confirm, then reloads data.
- [x] **Export CSV in-TUI** — run export logic in a Textual worker thread, show `notify()` toast on success/fail. No terminal hand-off.

### Phase 2 — Edit Issue Modal

- [x] **`EditIssueScreen(ModalScreen)`** — `Input` widgets pre-filled with selected issue's fields (issue #, LGY #, pub date, story title, writer, artist, rating). Submit calls `db.update_issue()`, dismiss reloads data.

### Phase 3 — Config Screen

- [x] **`ConfigScreen(Screen)`** — pushed via `c` key. Shows masked API key, DB path, cache TTL. API key input + "Validate & Save" button (validates via `comicvine.validate_api_key()` in a worker thread).

### Phase 4 — Add Issue Wizard *(depends on Phase 2 for field pattern)*

- [x] **`AddIssueScreen` multi-step wizard** — all in Textual, no suspend:
...
- [x] **`CLAUDE.md`** — add Textual to stack, note InquirerPy retained for `menu.py` test coverage, update key commands
- [x] **`AGENTS.md`** — update tech stack table and file structure reference diagram
- [x] **`README.md`** — update intro and dependencies section
- [x] **`tui.py` cleanup** — remove remaining `suspend()` stubs + dead `menu.py` imports once all phases ship
